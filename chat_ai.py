# -*- coding: utf-8 -*-
"""
Created on Tue May 10 22:40:35 2022
...
#importing dataset 
lines=open('chat_line.txt', encoding='utf-8', errors='ignore').read().split('\n')
conversation=open('chat_con.txt', encoding='utf-8', errors='ignore').read().split('\n')
...
#creating a list of conversation ids
conversation_ids=[]
for con in conversation[:-1]:
    _con=con.split(' +++$+++ ')[-1][1:-1].replace("'","").replace(" ","")
    conversation_ids.append(_con.split(","))
    
#getting answers and questions separatly
questions=[]
answers=[]
for con in conversation_ids:
...
#cleaning text
def clean_text(text):
    text=text.lower()#turn into lower cases
...
# Create two dictionaries that map anwers and question words into unique dictionaries.(NEED TO Change for Project )
...
#Creating a invers dictionary for answrs2int dic
...
#and replace all the word that filtered out by <OUT>
...
#Sorting question and answers by the lenght
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, questions_num_words, encoder_embedding_size, decoder_embedding_size, rnn_size, num_layers, questions2int):
    ...
    preprocessed_targets = preprocess_targets(targets, question2int, batch_size)
...
# Converting the questions from strings to lists ogoddf encoding integers
...
while(True):
    ...
    if question == 'Goodbye':
        break
    question = convert_string2int(question, question2int)
    question = question + [question2int['<PAD>']] * (25 - len(question))
